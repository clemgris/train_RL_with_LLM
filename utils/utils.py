...
def concatenate_transitions(transitions: List[Transition]) -> Transition:
    ...
    dict_res['done'] = jnp.concatenate([trans.done[None] for trans in transitions], axis=0)
    dict_res['action'] = jnp.concatenate([trans.action[None] for trans in transitions], axis=0)
    dict_res['value'] = jnp.concatenate([trans.value[None] for trans in transitions], axis=0)
    dict_res['reward'] = jnp.concatenate([trans.reward[None] for trans in transitions], axis=0)
    dict_res['log_prob'] = jnp.concatenate([trans.log_prob[None] for trans in transitions], axis=0)
